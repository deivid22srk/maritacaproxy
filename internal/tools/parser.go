...
var (
	toolOpenRE    = regexp.MustCompile(`<tool_call\b[^>]*>`)
	toolEnd       = `</tool_call>`
	toolShortEnd  = `</tool>`
	...
	xmlNameAttrRE = regexp.MustCompile(`(?is)<tool_call\b[^>]*\bname\s*=\s*["']([^"']+)["']`)
	xmlNameTagRE  = regexp.MustCompile(`(?is)<name>([\s\S]*?)</name>`)
	envDetailsRE  = regexp.MustCompile(`^\s*<environment_details\b`)
)

// StreamingToolParser incrementally parses <tool_call> tags from streamed text.
...
func findToolEndMatch(buffer string) *endMatch {
	...
	for i := 0; i < len(buffer); i++ {
		...
		// Truncated close variants
		for _, truncated := range []string{"</tool", "</"} {
			if matchesAt(buffer, i, truncated) && i+len(truncated) < len(buffer) && buffer[i+len(truncated)] == '<' {
				after := buffer[i+len(truncated):]
				if envDetailsRE.MatchString(after) {
					return &endMatch{i, len(truncated)}
				}
			}
		}
		if matchesAt(buffer, i, "</tool") {
			next := byte(0)
			if i+len("</tool") < len(buffer) {
				next = buffer[i+len("</tool")]
			}
			if next != 0 && next != '_' && next != '>' {
				return &endMatch{i, len("</tool")}
			}
		}
	}
	return nil
